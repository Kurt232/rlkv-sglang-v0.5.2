...
def generate_base_sparsity_mask(
    ...
    nrow, ncol = (
        round_to_multiple(max_seqlen_q, round_base) // m_block_dim,
        round_to_multiple(max_seqlen_k, round_base) // n_block_dim,
    )
    base_mask = torch.zeros(
        batch_size, num_blocksparse_heads, nrow, ncol, device=device, dtype=torch.bool
    )
            ...
            if not sparsity == 0.0 and not sparsity == 1.0:
                for i in range(nrow):
                    ...
                    if causal:
                        ...
                        base_mask[batch][head_rank][
                            idx, torch.randperm(available_col_num)[:num_one]
                        ] = True
                    else:
                ...
                base_mask[batch][head_rank] = torch.ones_like(
                    base_mask[batch][head_rank]
                )
...
def generate_streaming_mask(
    ...
    base_mask = torch.zeros(
        batch_size, num_heads, nrow, ncol, device=device, dtype=torch.bool
    )
    ...
    for batch in range(batch_size):
        ...
        for head_rank in range(num_heads):
            ...
            for i in range(start_row_idx, nrow):
                if causal:
                    max_row_block_num = (
                        ceil_div(max(actual_k - actual_q, 0), n_block_dim)
                        + 1
                        + i
                        - start_row_idx
                    )
...
def prepare_mixed_mask(
    base_blocksparse_mask,
    base_streaming_mask,
    head_mask_type,
    batch_size,
    num_heads,
    ...
    for head_rank in range(num_heads):
        ...
        if mask_type == 0:
            mixed_mask[:, head_rank, :, :] = torch.ones_like(
                mixed_mask[:, head_rank, :, :]
            )
        elif mask_type > 0:
    ...
    mixed_mask = repeat(
        mixed_mask,
        "b h s_m s_n -> b h (s_m d_m) (s_n d_n)",
        d_m=m_block_dim,
        d_n=n_block_dim,
    )
...
def tailor_mixedmask_for_test(spanded_base_mixedmask, seqlen_q, seqlen_k):
    ...
    pad_blockmask = torch.zeros(
        batch_size,
        nheads,
        seqlen_q,
        seqlen_k,
        dtype=torch.bool,
        device=spanded_base_mixedmask.device,
    )
    pad_blockmask[
        :, :, : spanded_base_mixedmask.shape[2], : spanded_base_mixedmask.shape[3]
    ] = spanded_base_mixedmask
