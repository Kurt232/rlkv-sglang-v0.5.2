...
class MixedNativeAttnBackend(AttentionBackend):
    ...
    def _run_sdpa_streaming_forward_decode(
        self,
        query: torch.Tensor,
        output: torch.Tensor,
        k_cache: torch.Tensor,
    ...
    def forward_decode(
        ...
        q_ = q.view(-1, layer.tp_q_head_num, layer.qk_head_dim)
        o_full_ = o_full.view(-1, layer.tp_q_head_num, layer.v_head_dim)
        o_streaming_ = o_streaming.view(-1, layer.tp_q_head_num, layer.v_head_dim)
        ...
        self._run_sdpa_streaming_forward_decode(
            q_,
            o_streaming_,
            forward_batch.token_to_kv_pool.get_key_buffer(layer.layer_id),
            forward_batch.token_to_kv_pool.get_value_buffer(layer.layer_id),
