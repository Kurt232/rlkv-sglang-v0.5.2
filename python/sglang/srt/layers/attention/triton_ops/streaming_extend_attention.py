...
def _fwd_kernel(
    ...
    local_win_start_idx = tl.maximum(cur_seq_len_prefix - LOCAL_WINDOW_SIZE, 0)
    ...
    for start_n in range(0, cur_seq_len_prefix, BLOCK_N):
        ...
        if SINK_WINDOW_SIZE > 0:
            sink_mask = start_n + offs_n < SINK_WINDOW_SIZE
        else:
            sink_mask = False
        if LOCAL_WINDOW_SIZE > 0:
            local_mask = (
                cur_seq_len_prefix + (cur_block_m * BLOCK_M + offs_m[:, None])
                < start_n + offs_n + LOCAL_WINDOW_SIZE
            )
        else:
            local_mask = False
    ...
    # stage 2: compute the triangle part (current sequence)
    ...
    for start_n in range(0, cur_block_m_end, BLOCK_N):
        ...
        final_mask = mask_m[:, None] & mask_n[None, :]
        if LOCAL_WINDOW_SIZE > 0:
            local_mask = (cur_block_m * BLOCK_M + offs_m[:, None]) < (
                start_n + offs_n[None, :] + LOCAL_WINDOW_SIZE
            )
...
def streaming_extend_attention_fwd(
    q_extend,
    k_extend,
    v_extend,
    o_extend,
    ...
    """
    assert (
        sink_win_size > 0 or local_win_size > 0
    ), "At least one of sink_win_size or local_win_size must be greater than 0"
    ...
    assert (
        sink_win_size % BLOCK_N == 0 and local_win_size % BLOCK_N == 0
    ...
    _fwd_kernel[grid](
        ...
        v_buffer.stride(0),
        v_buffer.stride(1),
        SINK_WINDOW_SIZE=sink_win_size,
